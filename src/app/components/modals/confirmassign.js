        ...
        <div className="flex flex-col gap-5 animate-in fade-in duration-300">
                ...
                <div className="flex flex-wrap gap-2 min-h-[40px] p-2.5 rounded-lg bg-slate-50 border border-dashed border-slate-200">
                    {selectedUser.length > 0 ? (
                        selectedUser.map((user) => (
                            <div key={user.id} className="flex items-center gap-1.5 bg-white border border-indigo-100 text-slate-700 rounded-md px-2 py-1 animate-in zoom-in duration-200">
                        ...
                        <p className="text-xs text-slate-400 px-1 py-0.5">No users selected yet...</p>
                    )}
                </div>

                <div className="space-y-1.5">
                    <label className="text-xs font-semibold text-slate-600">Search Sales Persons</label>
            ...
            <div className="flex items-center gap-3 pt-4 border-t border-slate-100">
                ...
                <button
                    ...
                    type="button"
                    className="flex-[1.5] px-4 py-2.5 bg-indigo-600 disabled:opacity-50 text-white rounded-lg text-sm font-medium hover:bg-indigo-700 transition-colors flex items-center justify-center gap-2"
                >
