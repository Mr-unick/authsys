        ...
        <div className="flex flex-col gap-6 animate-in fade-in duration-500">
                ...
                <div className="flex flex-wrap gap-2 min-h-[40px] p-2 rounded-xl bg-gray-50/50 border border-dashed border-gray-200">
                    {selectedUser.length > 0 ? (
                        selectedUser.map((user) => (
                            <div key={user.id} className="flex items-center gap-1.5 bg-white border border-indigo-100 text-[#0F1626] rounded-lg px-2 py-1 shadow-sm animate-in zoom-in duration-300">
                        ...
                        <p className="text-[10px] text-gray-400 italic px-2 py-1">No users selected yet...</p>
                    )}
                </div>

                <div className="space-y-2">
                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-1">Search Sales Persons</label>
                    <AutocompleteComponent
                        ...
                        renderItem={(user) => (
                            <div className="flex items-center justify-between w-full px-1">
                                ...
                                {selectedUser.find(u => u.id === user.id) && <Check size={12} className="text-indigo-600" />}
                            </div>
                        )}
            ...
            <div className="flex items-center gap-3 pt-4 border-t border-gray-50">
                ...
                <button
                    ...
                    type="button"
                    className="flex-[1.5] px-4 py-3 bg-[#0F1626] disabled:opacity-50 disabled:grayscale text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-indigo-600 transition-all shadow-lg shadow-indigo-100 flex items-center justify-center gap-2"
                >
