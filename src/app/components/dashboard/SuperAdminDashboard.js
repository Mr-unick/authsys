    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500 pb-20">
            ...
            <div className="flex items-center gap-4">
                <div className={`w-11 h-11 rounded-2xl ${isBusinessAdmin ? 'bg-blue-600' : 'bg-indigo-600'} flex items-center justify-center shadow-lg ${isBusinessAdmin ? 'shadow-blue-200' : 'shadow-indigo-200'}`}>
                    ...
                    >
                        <div className="p-6 grid grid-cols-2 sm:grid-cols-4 gap-4">
                            {data.charts.pipeline?.data.map((stage, idx) => (
                                <div key={idx} className="flex flex-col items-center gap-2 p-4 bg-slate-50 rounded-xl border border-slate-100 hover:bg-slate-100 transition-colors">
                                    <div className="w-3 h-3 rounded-full" style={{ backgroundColor: stage.color }} />
                                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider text-center">{stage.label}</span>
                                    ...
                                    {data.charts.ticketStatus?.data.map((entry, idx) => (
                                        <div key={idx} className="flex items-center justify-between bg-slate-50 rounded-lg px-3 py-2 border border-slate-100">
                                            <div className="flex items-center gap-1.5">
                                                ...
                                                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-tight">{entry.label}</span>
                                            </div>
                            ...
                            {(isBusinessAdmin ? data.branches.recent : data.tenants.recent).map((item) => (
                                <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                                    ...
                                    <td className="px-6 py-4">
                                        <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border ${isBusinessAdmin
